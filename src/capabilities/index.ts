...
async function dispatch(name: string, input: unknown, onOutput?: (chunk: string) => void): Promise<string> {
...
export function createToolExecutor(): ToolExecutor {
  return async (name: string, input: unknown, onOutput?: (chunk: string) => void): Promise<string> => {
    let result: string
    try {
      result = await dispatch(name, input, onOutput)
    } catch (err) {
      // A throwing capability must not crash the agent loop — feed the error
      // back to the model as the tool result so it can recover.
      return `Tool "${name}" failed: ${err instanceof Error ? err.message : String(err)}`
    }
    // Bound every tool result so one oversized output can't blow up the context.
